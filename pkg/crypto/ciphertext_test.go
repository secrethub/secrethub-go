...
func (d *UnknownCiphertext) Decrypt(k Key) ([]byte, error) {
	return nil, nil
}

func (d *UnknownCiphertext) ReEncrypt(k1, k2 Key) (Ciphertext, error) {
	return nil, nil
}
...
func TestEncodeCiphertext(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			output:        EncodedCiphertext("RSA-OAEP+AES-GCM$YWVzX2RhdGE=$key=cnNhX2RhdGE=,nonce=bm9uY2VfZGF0YQ=="),
			expectSuccess: true,
		},
		{
			input: &CiphertextAES{
			...
			output:        EncodedCiphertext("AES-GCM$YWVzX2RhdGE=$nonce=bm9uY2VfZGF0YQ=="),
			expectSuccess: true,
		},
		{
			input: &CiphertextRSA{
			...
			output:        EncodedCiphertext("RSA-OAEP$cnNhX2RhdGE=$"),
			expectSuccess: true,
		},
		{
			input:         &CiphertextRSA{},
	...
	for _, test := range tests {
		b, err := EncodeCiphertext(test.input)
